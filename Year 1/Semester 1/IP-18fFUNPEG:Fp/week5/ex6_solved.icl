...
triples :: [Int] -> [Int]
//triples lst = map ((*)3)lst
triples lst = map (\c = c * 3)lst
...
lastdel :: [[Int]] -> [[Int]]
lastdel lst = map(\x = take (length(x)-1) x) lst
...
//avg = Sum of List / length of List  => answer is Real number
avg :: [Real] -> Real
avg lst = (foldr(+) 0.0 lst) / toReal(length lst)
...
//we need list of elements index

// findInd :: Int [Int] -> Int
// findInd x ls = [i \\ i <- ls & i <- [0..] | ls == x ]

// Start = findInd 10 [1..100]

// alternatingSum :: [Int] -> Int
// alternatingSum = filter - filter
