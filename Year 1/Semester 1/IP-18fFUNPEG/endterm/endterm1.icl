...
arrayToList c = removeDup [x \\ x <-: c]


helperTwo :: Char String -> (Char,Int)
helperTwo c arr = (c, length [x \\ x <-: arr | x == c])


subHelper :: [Char] String -> {(Char,Int)}
subHelper findC arr = {helperTwo x arr \\ x <- findC | snd( helperTwo x arr) > 1}



count :: String -> {(Char, Int)} 
count arr = subHelper (arrayToList arr) arr
...
instance - [Int]
where
    (-) l1 l2 = [x \\ x <- l1 | (isMember x l2) == False ]
...
Insert (Pointer x child) str = Pointer x (Insert child str)
...
rowEl :: [Int] Int -> Int 
rowEl n i  = sum [x \\ x <- n & j <-[1..i] | j == i]

maxMat :: [[Int]] -> Int 
maxMat lst = sum [rowEl x i  \\ x <- lst & i <- [1..(length lst)]]


largestMat :: {[[Int]]} -> [[Int]]
largestMat arr = hd [ x \\ x<-:arr  | maxMat x == maxList[maxMat x \\ x <-: arr]]
