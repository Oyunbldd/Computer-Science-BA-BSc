...
helper :: {Int} -> [Int]
helper arr =  removeDup [x \\ x<-: arr]


howMany :: Int {Int} -> (Int , Int)
howMany n arr = (n , length [x \\ x<-:arr | x == n]) 

printf :: (Int, Int) -> [Int]
printf n = [fst(n) \\ i <- [1..snd(n)]]
...
removeOcc :: {Int} -> {Int}
removeOcc arr = lstToArr (flatten[printf (howMany x arr) \\ x <-helper arr | snd(howMany x arr) rem 2 <> 0])
...
generate :: Int -> {Int}
generate n = lstToArr (flatten[manyTimes i \\ i <- [1..n] ])
