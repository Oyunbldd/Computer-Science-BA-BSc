...
shift :: Char Int -> Char 
shift c n 
| x > 90 = toChar (64 + x rem 90)
= toChar (x)
where x = toInt c + n

shiftCipher:: String Int -> String
shiftCipher arr n = toString [ shift x n \\ x<-: arr ]
...
distance :: Point Point Real-> Bool 
distance cc p r
| sqrt ((cc.x - p.x) * (cc.x - p.x) + (cc.y - p.y) *(cc.y - p.y)) < r = True
= False

check :: Circle {Point} -> Bool
check c p = length [x \\ x <-: p | distance c.center x c.radius] > 0
