...
oneHelper :: [Int] -> Int
oneHelper x = sum [i^j \\ i <-x  & j <-[1..]]


isDisariumNum :: Int -> Bool
isDisariumNum x 
| x == oneHelper (reverse (intToList x)) = True
= False
...
harshadNums :: [Int] -> [Int]
harshadNums lst = [x \\ x <- lst | x rem (sum (intToList x)) == 0]
...
threeHelper :: Int [Int] -> Bool
threeHelper n lst 
| length [i \\ i<-lst | n > i] == length lst = True
= False

leaders :: [Int] -> [Int]
leaders [] = []
leaders [x : xs]
| threeHelper x xs = [x] ++ leaders xs
= leaders xs
...
filteredRem n lst = [i rem n \\ i <- lst | i rem n <> 0 ]
...
countGoodNums lst = length [x \\ x <- lst | x >= 10 && x<= 99 && x rem 3 == 0]
...
possible tp 
| (fst3 tp  + snd3 tp > thd3 tp) && (fst3 tp + thd3 tp > snd3 tp ) && (snd3 tp + thd3 tp > fst3 tp) = True
= False 

validTriangles :: [(Int,Int,Int)] -> [Bool]
validTriangles lst = [possible x \\ x <-lst ]
...
manyTimes :: (String, Int) -> [String]
manyTimes tp 
| snd tp <= 0 = []
= [fst tp \\ x <- [1..(snd tp)]]

stringCopy :: [(String,Int)] -> [[String]]
stringCopy lst = [manyTimes x \\ x <- lst]
...
listToStr lst = [toString(x) \\ x<-lst]

strToInt :: [String] -> String
strToInt [] = ""
strToInt [x : xs] = x +++ strToInt xs
...
intInsertion x y =toInt (strToInt [i+++j \\ i <-listToStr (reverse (intToList x)) & j <-listToStr (reverse (intToList y))])
...
countVowel :: [Char] -> Int 
countVowel lst = sum [1 \\ i <- lst | i == 'a' || i == 'e' || i == 'o' || i == 'u' || i == 'i']




tenHelper :: Char Int -> Char
tenHelper c n = toChar ((toInt c) + n)

cipherList :: [Char] -> [Char]
cipherList lst = [tenHelper c (countVowel lst) \\ c <- lst]
...
evaluate lst n = sum[x*(n^i) \\ x<-lst & i <-[0..] ]
...
// Start = zip ([1,2,3],[11,22,33])
// Start = dropWhile (isOdd) [1,1,3,4]
