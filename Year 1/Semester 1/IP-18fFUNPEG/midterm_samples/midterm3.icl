...
numberToList x 
| x < 10 = [x]
= [x rem 10] ++ numberToList ( x / 10)


pHelper :: [Int] [Int] -> Bool
pHelper [][] = True
pHelper [x : xs] [y : ys]
...
// Start = pHelper [1,2] [1,2]


parasitic :: Int Int -> Bool
parasitic x y = pHelper (sort (numberToList x))  (sort (numberToList (x*y)))
...
| n < 10  && n rem 10 == 1 = 1
| n < 10  && n rem 10 <> 1 = 0
| n rem 10 == 1 = 1 + checkDouble (n/10)
= checkDouble (n / 10)


doubleOne :: [Int] -> [Int]
doubleOne lst = [x \\  x <- lst | (checkDouble x) >= 2]
...
repHelper :: [Int] Int  Int-> [Int]
repHelper lst mid x = updateAt mid x lst


// Start =repHelper [0,1,3,4] 2 100

repMid :: [[Int]] Int -> [[Int]]
repMid lst n = map(\x = repHelper x ((length x) / 2) n ) lst
...
checkPrime :: Int -> Bool
checkPrime n = length [i \\ i <- [1..n] | (n rem i) == 0] == 2

primes7 :: [Int] -> [Int]
primes7 lst = [x \\ x <- lst | checkPrime x && x rem 10 == 7]
...
holdsHelper :: (Int,Int) -> Bool
holdsHelper tp 
| fst tp rem 2 == 0 && snd tp rem 2 == 1 = True
| fst tp rem 2 == 1 && snd tp rem 2 == 0 = True
= False


holdsTrue :: [(Int, Int)] -> Bool
holdsTrue [] = False
holdsTrue lst = length (filter ((==) False) (map (\x = holdsHelper x)lst)) == 0
...
super_digit :: [Int] -> [Int]
super_digit lst = map (\x = totalSum x ) lst
...
checkPower :: Int Int -> Bool 
checkPower x 1 = True
checkPower x n
| n rem x == 0 = checkPower x (n / x)
= False


powersList :: [Int] Int -> [Int]
powersList lst n = [x \\ x <- lst | checkPower n x  ]
...
rangedPrime :: Int Int -> [Int]
rangedPrime x y = [i \\ i <- [x..y] | checkPrime i]
...
helperFunction [x : xs]
| length xs == 0 = 0
| (hd xs) - x <= 2 = 1 + helperFunction xs
= helperFunction xs



twinPrimes :: Int Int -> Int
twinPrimes x y = helperFunction(rangedPrime x y)
