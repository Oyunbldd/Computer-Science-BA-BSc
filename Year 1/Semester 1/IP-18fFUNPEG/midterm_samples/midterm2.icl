...
// filter  -> length of list map utga tentseuu bol lengtheer solino

howMany :: Int [Int] -> Int
howMany n lst = length (filter((==) n) lst)


occNum :: [Int] -> [Int]
occNum lst = [howMany x lst \\ x <- lst]
...
subFunction [] = []
subFunction [x : xs ]
| isPalindrom x == True = subFunction xs
= [ x : subFunction xs]


getRidPal :: [[Int]] -> [[Int]]
getRidPal lst = map (\x = subFunction x) lst
...
removeNotPrime lst =  [x \\ x <- lst | isPrime x == False]
...
*/
// checkingIsGood :: [Int] [Int] -> [Int]
// checkingIsGood [] = True
// checkingIsGood lstO lstT = 
// [x \\ x <- lst & i <-[0,2.] | x rem 2 == 0 ] ++ [x \\ x <- lst & i <-[1,3..] | x rem 2 == 1]
...
helperFunction lst 
| sum lst > 10 = lst ++ reverse lst
= []

symSumGreater10 :: [[Int]] -> [[Int]]
symSumGreater10 lst = [ helperFunction x \\ x <- lst | helperFunction x <> []]
