...
check :: Int [Int] -> Bool
check n lst = length (filter(\x = x == n)lst) >=5

famousNum :: [Int] -> [Int]
famousNum lst = [x \\ x <-lst | check x lst]
...
findPrev n lst 
| hd lst == n = -1
| isMember n lst  = last (takeWhile(\c = c <> n)lst)
= -1
...
symHelper :: Int [Int] -> Bool
symHelper n lst
| length (filter(\x = x == n) lst) >= 2 = False
= True

symmetricDif :: [Int] [Int] -> [Int]
symmetricDif ll rl = [x \\ x <- list | symHelper x list]
...
testing [] lst = 0
testing [x : xs] lst
| isMember x lst = 1 + testing xs lst
= testing xs lst
...
goodLists [] list = 0
goodLists [x : xs] list
| testing x list >= (length x) / 2 = 1 + goodLists xs list 
= goodLists xs list
...
helperFunction :: Int Int Int Int -> Int
helperFunction n a b c
| n == 0 = a 
| n == 1 = b
| n == 2 = c
= ((helperFunction (n-1) a b c) * (helperFunction (n-2) a b c) + (helperFunction  (n-3) a b c)) rem 1000



clean :: Int Int Int Int -> [Int] 
clean n a b c  = [helperFunction x a b c \\ x <- [0..n]]
