...
helperOne :: [Int] -> Int
helperOne lst = sum[x ^ i \\ x <-lst & i <-[1..]]

isDisariumNum :: Int -> Bool
isDisariumNum x 
| helperOne(numToList x) == x = True
= False


// Start = isDisariumNum 135 // True
// Start = isDisariumNum 598 // sTrue
...
harshadNums [] = []
harshadNums [ x : xs]
| x rem (sum (numToList x)) == 0  = [x : harshadNums xs]
= harshadNums xs
...
threeHelper n [] = True
threeHelper n [x : xs]
| n > x = threeHelper n xs
= False

leaders :: [Int] -> [Int]
leaders [] = []
leaders [x : xs] 
| threeHelper x xs == True = [x : leaders xs]
= leaders xs
...
filteredRem n lst = map(\i= i rem n)[x \\ x <- lst | x rem n <> 0]
...
countGoodNums lst = length [x \\ x <- lst | x >= 10 && x <= 100 && x rem 3 == 0]
...
validTriangles lst = map(\x = sixHelper x)lst
...
stringCopy lst = map(\x = [fst x \\ i <- [1..(snd x)]]) lst
...
intHelper :: Int Int -> [[Int]]
intHelper one two = [[x]++[y] \\ x <-numToList one & y <-numToList two]

// Start = flatten (intHelper 111 222)

intInsertion :: Int Int -> Int
intInsertion one two = ListtoNum (flatten (intHelper one two))
...
//---------------
moverHelper :: [Int] Int -> Int
moverHelper lst x = (length lst) - length(dropWhile ((<>) x) lst) 
                    //3                        // 1 



Mover :: Int Int Int -> Int
Mover x y z = (moverHelper (numToList z) y) - (moverHelper (numToList x) y)
