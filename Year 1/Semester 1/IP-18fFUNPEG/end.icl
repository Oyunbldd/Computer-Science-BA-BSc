...
oneHelper :: [Char] Char -> Int
oneHelper lst c = hd [i \\ x <-lst & i <- [0..] | x == c]
...
isValid :: String -> Bool
isValid str 
| isMember '@' lst <> True = False
| isMember '.' lst <> True = False
| (oneHelper lst '.') == (length lst - 1) = False

= True

where lst = strToChar str 
...
secondHelper :: String Char -> String
secondHelper lst c = toString [x \\ x <-: lst  | x <> c ]


removeChar :: [String] -> [String]
removeChar lst = [secondHelper x (x.[2]) \\ x <-lst]
...
*/
sumHand :: [Card] -> Int
sumHand lst = sum[returner l.rank\\l<-lst]
...
countDepth :: Tree -> Int
countDepth Leaf = 0
countDepth (Node le ri) = 1 + (countDepth le) + (countDepth ri)


balance :: Tree -> Int
balance (Node le ri) 
| (countDepth le) > (countDepth ri) = -1
| (countDepth le) < (countDepth ri) = 1
= 0
...
instance * [Int] 
 where 
   (*) lst1 lst2 = sort (removeDup(lst1 ++ lst2))
...
//changeVal :: Sea -> Sea
//changeVal lst 
//| lst.numOfShark > 15 = {name = lst.name , numOfShark = lst.numOfShark , statistics = Average}
//| lst.numOfShark < 5 && lst.numOfShark > 15 = {name = lst.name , numOfShark = lst.numOfShark , statistics = Average}
//= {name = lst.name, numOfShark = lst.numOfShark , statistics = Low}

//eightHelper :: Ocean -> Ocean
//eightHelper lst = {changeVal x \\ x <-:lst.seas}

//updateStatistics :: {Ocean} -> {Ocean}
//updateStatistics lst = {eightHelper \\ x <-:lst}
